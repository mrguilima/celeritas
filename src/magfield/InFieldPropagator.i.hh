...
bool InFieldPropagator::operator()() const
{
  ...
  FieldLookup::get_field_value(Position, BfieldInitial, bmag);
  double step_field = Max(this->safe_length(track_, gEpsDeflection, BfieldInitial, bmag),
                          track_.safety());

  double step = Min(step_geom_phys, step_field);

  // Propagate in magnetic field
  std::cout<<" FieldPropH: Propagate(): spot 3: pos="<< track_ <<"\n";
  propagate_in_volume(track_, step, BfieldInitial, bmag);
  std::cout<<" FieldPropH: Propagate(): spot 4: pos="<< track_ <<"\n";
...
// #  warning "Propagation has no way to tell scheduler what to do yet."
  ...
  // physics process
  if (track_.status() == SimulationStage::Physics) {
    // Update number of steps to physics and total number of steps
    // td->fNphys++;
    // td->fNsteps++;
    std::cout<<" FieldPropH: Propagate(): spot 5: pos="<< track_ <<"\n";
    track_->set_stage(stageAfterCrossing); // Future: (kPostPropagationStage);
    std::cout<<" FieldPropH: Propagate(): spot 5a: pos="<< track_ <<"\n";
  } else {
    // Crossing tracks continue to continuous processes, the rest have to
    // query again the geometry
    std::cout<<" FieldPropH: Propagate(): spot 6: pos="<< track_ <<"\n";
    if ((track_->get_safety() < 1.E-10) && !IsSameLocation(*tracks)) {
      std::cout<<" FieldPropH: Propagate(): spot 6a: pos="<< track_ <<"\n";
      // td->fNcross++;
      // td->fNsteps++;
    } else {
      std::cout<<" FieldPropH: Propagate(): spot 6b: pos="<< track_ <<"\n";
      track_->SetStage(SimulationStage::GeometryStepStage);
      std::cout<<" FieldPropH: Propagate(): spot 6c: pos="<< track_ <<"\n";
    }
  }
#endif
                                         ...
                                         double epsilon) const
{
  ...
  /*
VECCORE_ATT_HOST
void InFieldPropagator::PrintStats() const
{
#ifdef STATS_METHODS
  unsigned long nTot = numTot;
