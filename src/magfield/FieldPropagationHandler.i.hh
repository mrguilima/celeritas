...
//---------------------------------------------------------------------------//
//! \file FieldPropagatorHandler.i.hh
//---------------------------------------------------------------------------//
					    ...
					    double bmag) const
{
  ...
  double plong = pmag * track.dir().Dot(Bfield) / bmag;
...
bool FieldPropagationHandler::Propagate(GeoTrackView &track) const
{
  ...
  ThreeVector Position(track.pos());
...
#define PRINT_STEP_SINGLE 1
  ...
  double curvaturePlus = fabs(kB2C * Charge(track) * (bmag * toKiloGauss)) / (track.P() + 1.0e-30); // norm for step
  const double angle = crtstep * curvaturePlus;
  std::cout<<"__PropagateInVolume(Single): Momentum= "<< track.P() <<"/"<< units::GeV <<"="<< (track.P()/units::GeV)
...
// Print("\n");
  ...
  const ThreeVector_t Position(track.pos());
  const ThreeVector_t Direction(track.dir());
...
#else
  track.pos() = PositionNew;
  track.dir() = DirectionUnit;
  // TODO: need to add normlization of track.
  // Normalize(track)
  //track.dir().Normalize(); // GL: Not needed, DirectionUnit is normalized.
